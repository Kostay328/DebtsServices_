package ru.ining.debts.sts.merge.pojo.DB;


import java.sql.Timestamp;

public class NewDebts {

  private String vclstamp;
  private String vcl;
  private String tcard;
  private String reason;
  private String ordinance;
  private java.sql.Timestamp dbtdte;
  private java.sql.Timestamp ofndte;
  private String sum;
  private String sumHalf;
  private java.sql.Timestamp paytodte;
  private java.sql.Timestamp paytoHalf;
  private String brn;
  private String brnname;
  private String regnum;
  private String place;
  private String article;
  private java.sql.Timestamp paydte;
  private String rcpnum;
  private java.sql.Timestamp mgrpaydte;
  private String mgrfactsum;
  private String mgrcomment;
  private String mgrpayedbybrn;
  private String svidReg;
  private java.sql.Timestamp data;
  private String inn;
  private String ufk;
  private String kpp;
  private String summaStrafa;
  private String raschSchet;
  private String bik;
  private String bankPol;
  private String kbk;
  private String oktomo;
  private String naznachenie;
  private String entby;
  private java.sql.Timestamp entdte;
  private String lstchgby;
  private java.sql.Timestamp lstchkdte;
  private java.sql.Timestamp lstchgdte;
  private String srvCode;
  private String rcdsts;
  private String id;

  public NewDebts() {
    this.vclstamp = "";
    this.vcl = "";
    this.tcard = "";
    this.reason = "";
    this.ordinance = "";
//    this.dbtdte = ;
//    this.ofndte = ofndte;
    this.sum = "0";
    this.sumHalf = "0";
//    this.paytodte = paytodte;
//    this.paytoHalf = paytoHalf;
    this.brn = "0";
    this.brnname = "";
    this.regnum = "";
    this.place = "";
    this.article = "";
//    this.paydte = paydte;
    this.rcpnum = "";
//    this.mgrpaydte = ;
    this.mgrfactsum = "";
    this.mgrcomment = "";
    this.mgrpayedbybrn = "";
    this.svidReg = "";
//    this.data = ;
    this.inn = "";
    this.ufk = "";
    this.kpp = "";
    this.summaStrafa = "";
    this.raschSchet = "";
    this.bik = "";
    this.bankPol = "";
    this.kbk = "";
    this.oktomo = "";
    this.naznachenie = "";
    this.entby = "";
//    this.entdte = entdte;
    this.lstchgby = "";
//    this.lstchkdte = lstchkdte;
//    this.lstchgdte = lstchgdte;
    this.srvCode = "1";
    this.rcdsts = "0";
//    this.id = id;
  }

  public String getVclstamp() {
    return vclstamp;
  }

  public void setVclstamp(String vclstamp) {
    this.vclstamp = vclstamp;
  }


  public String getVcl() {
    return vcl;
  }

  public void setVcl(String vcl) {
    this.vcl = vcl;
  }


  public String getTcard() {
    return tcard;
  }

  public void setTcard(String tcard) {
    this.tcard = tcard;
  }


  public String getReason() {
    return reason;
  }

  public void setReason(String reason) {
    this.reason = reason;
  }


  public String getOrdinance() {
    return ordinance;
  }

  public void setOrdinance(String ordinance) {
    this.ordinance = ordinance;
  }


  public java.sql.Timestamp getDbtdte() {
    return dbtdte;
  }

  public void setDbtdte(java.sql.Timestamp dbtdte) {
    this.dbtdte = dbtdte;
  }


  public java.sql.Timestamp getOfndte() {
    return ofndte;
  }

  public void setOfndte(java.sql.Timestamp ofndte) {
    this.ofndte = ofndte;
  }


  public String getSum() {
    return sum;
  }

  public void setSum(String sum) {
    this.sum = sum;
  }


  public String getSumHalf() {
    return sumHalf;
  }

  public void setSumHalf(String sumHalf) {
    this.sumHalf = sumHalf;
  }


  public java.sql.Timestamp getPaytodte() {
    return paytodte;
  }

  public void setPaytodte(java.sql.Timestamp paytodte) {
    this.paytodte = paytodte;
  }


  public java.sql.Timestamp getPaytoHalf() {
    return paytoHalf;
  }

  public void setPaytoHalf(java.sql.Timestamp paytoHalf) {
    this.paytoHalf = paytoHalf;
  }


  public String getBrn() {
    return brn;
  }

  public void setBrn(String brn) {
    this.brn = brn;
  }


  public String getBrnname() {
    return brnname;
  }

  public void setBrnname(String brnname) {
    this.brnname = brnname;
  }


  public String getRegnum() {
    return regnum;
  }

  public void setRegnum(String regnum) {
    this.regnum = regnum;
  }


  public String getPlace() {
    return place;
  }

  public void setPlace(String place) {
    this.place = place;
  }


  public String getArticle() {
    return article;
  }

  public void setArticle(String article) {
    this.article = article;
  }


  public java.sql.Timestamp getPaydte() {
    return paydte;
  }

  public void setPaydte(java.sql.Timestamp paydte) {
    this.paydte = paydte;
  }


  public String getRcpnum() {
    return rcpnum;
  }

  public void setRcpnum(String rcpnum) {
    this.rcpnum = rcpnum;
  }


  public java.sql.Timestamp getMgrpaydte() {
    return mgrpaydte;
  }

  public void setMgrpaydte(java.sql.Timestamp mgrpaydte) {
    this.mgrpaydte = mgrpaydte;
  }


  public String getMgrfactsum() {
    return mgrfactsum;
  }

  public void setMgrfactsum(String mgrfactsum) {
    this.mgrfactsum = mgrfactsum;
  }


  public String getMgrcomment() {
    return mgrcomment;
  }

  public void setMgrcomment(String mgrcomment) {
    this.mgrcomment = mgrcomment;
  }


  public String getMgrpayedbybrn() {
    return mgrpayedbybrn;
  }

  public void setMgrpayedbybrn(String mgrpayedbybrn) {
    this.mgrpayedbybrn = mgrpayedbybrn;
  }


  public String getSvidReg() {
    return svidReg;
  }

  public void setSvidReg(String svidReg) {
    this.svidReg = svidReg;
  }


  public java.sql.Timestamp getData() {
    return data;
  }

  public void setData(java.sql.Timestamp data) {
    this.data = data;
  }


  public String getInn() {
    return inn;
  }

  public void setInn(String inn) {
    this.inn = inn;
  }


  public String getUfk() {
    return ufk;
  }

  public void setUfk(String ufk) {
    this.ufk = ufk;
  }


  public String getKpp() {
    return kpp;
  }

  public void setKpp(String kpp) {
    this.kpp = kpp;
  }


  public String getSummaStrafa() {
    return summaStrafa;
  }

  public void setSummaStrafa(String summaStrafa) {
    this.summaStrafa = summaStrafa;
  }


  public String getRaschSchet() {
    return raschSchet;
  }

  public void setRaschSchet(String raschSchet) {
    this.raschSchet = raschSchet;
  }


  public String getBik() {
    return bik;
  }

  public void setBik(String bik) {
    this.bik = bik;
  }


  public String getBankPol() {
    return bankPol;
  }

  public void setBankPol(String bankPol) {
    this.bankPol = bankPol;
  }


  public String getKbk() {
    return kbk;
  }

  public void setKbk(String kbk) {
    this.kbk = kbk;
  }


  public String getOktomo() {
    return oktomo;
  }

  public void setOktomo(String oktomo) {
    this.oktomo = oktomo;
  }


  public String getNaznachenie() {
    return naznachenie;
  }

  public void setNaznachenie(String naznachenie) {
    this.naznachenie = naznachenie;
  }


  public String getEntby() {
    return entby;
  }

  public void setEntby(String entby) {
    this.entby = entby;
  }


  public java.sql.Timestamp getEntdte() {
    return entdte;
  }

  public void setEntdte(java.sql.Timestamp entdte) {
    this.entdte = entdte;
  }


  public String getLstchgby() {
    return lstchgby;
  }

  public void setLstchgby(String lstchgby) {
    this.lstchgby = lstchgby;
  }


  public java.sql.Timestamp getLstchkdte() {
    return lstchkdte;
  }

  public void setLstchkdte(java.sql.Timestamp lstchkdte) {
    this.lstchkdte = lstchkdte;
  }


  public java.sql.Timestamp getLstchgdte() {
    return lstchgdte;
  }

  public void setLstchgdte(java.sql.Timestamp lstchgdte) {
    this.lstchgdte = lstchgdte;
  }


  public String getSrvCode() {
    return srvCode;
  }

  public void setSrvCode(String srvCode) {
    this.srvCode = srvCode;
  }


  public String getRcdsts() {
    return rcdsts;
  }

  public void setRcdsts(String rcdsts) {
    this.rcdsts = rcdsts;
  }


  public String getId() {
    return id;
  }

  public void setId(String id) {
    this.id = id;
  }

}
